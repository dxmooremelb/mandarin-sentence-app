...
function applyFilter() {
  ...
  filtered = allCards.filter((card) => {
    ...
    const matchesFilter =
      ...
      (state.settings.filter === "due" &&
        (!progress.due || progress.due <= today()));
...
function markStudied(card, rating) {
  ...
  progress.due = new Date(now.getTime() + days * 86400000)
    .toISOString()
    .slice(0, 10);
...
function renderLevels() {
  ...
  $("levelList").innerHTML = levels
    .map((level) => {
      const levelCards = Object.keys(state.cards).filter((id) =>
        id.startsWith(`${level.id}:`),
      );
      const studied = levelCards.filter(
        (id) => state.cards[id]?.studied,
      ).length;
      const percent = level.count
        ? Math.round((studied / level.count) * 100)
        : 0;
      return `
...
async function saveOffline() {
  ...
  try {
    ...
    if (!manifestResponse.ok)
      throw new Error("Could not load offline asset list.");
    ...
    if (failed) {
      ...
      alert(
        `${failed} files could not be saved offline. Try again on a stronger connection.`,
      );
    } else {
...
function render() {
  ...
  $("position").textContent = filtered.length
  ...
  if (notes && card)
    notes.addEventListener("input", (e) => saveNote(card.id, e.target.value));
}

async function loadCards(levelId) {
  try {
    const res = await fetch(
      `${DATA_ROOT}/levels/${encodeURIComponent(levelId || "")}.json?t=${Date.now()}`,
    );
    ...
    current = Math.max(
      0,
      filtered.findIndex((card) => card.id === savedCard),
    );
    $("search").value = "";
...
async function loadLevels() {
  try {
    ...
    const data = await res.json();
    if (!res.ok || data.error)
      throw new Error(data.error || "Could not load levels.");

    levels = data.levels || [];
    selectedLevel =
      state.settings.lastLevel || data.defaultLevel || levels[0]?.id || "";
    if (!levels.length)
      throw new Error(`No level spreadsheets found in ${data.folder}.`);
    if (!levels.some((level) => level.id === selectedLevel))
      selectedLevel = data.defaultLevel || levels[0].id;
    renderLevels();
...
$("randomBtn").addEventListener("click", () =>
  goToCard(Math.floor(Math.random() * filtered.length)),
);
